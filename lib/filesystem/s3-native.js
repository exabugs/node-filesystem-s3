...
S3.prototype.createReadStream = function (key, callback) {
  ...
  s3.headObject({}, function (err, data) {
    if (err) {
      ...
      callback(null, s3.getObject().createReadStream());
    }
  });
};
 ...
 * @param callback
 */
S3.prototype.write = function (key, body, type, callback) {
  var s3 = open(this.bucket, key);
 ...
 *
 * @param key
 * @param params = [ {ext: 'small', width: 100, height: 100} ]
 * @param callback
 */
S3.prototype.resize = function (key, params, callback) {
  var self = this;

  var so0 = open(self.bucket, key);
  so0.headObject({}, function (err, data) {
    ...
    async.each(params, function (param, next) {
      ...
      var id = [key, param.ext].join('.');
      var file = path.join(os.tmpDir(), "tmp_" + id);
      gm(rs0).options({imageMagick: true}).resize(param.width, param.height).write(file, function (err) {
        var so1 = open(self.bucket, id);
