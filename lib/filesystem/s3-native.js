...
S3.prototype.getSignedUrl = function (key, callback) {
  var env = process.env;
  if (env.AWS_CF_CERT && env.AWS_CF_KEY && env.AWS_CF_HOST) {
    // CloufFrontリダイレクト
    var info = {
    ...
    var expires = (new Date()).getTime() + 3000;  // epoch-expiration-time
    var policy = {
      'Statement': [
        {
          'Resource': resource,
    ...
    fs.readFile(process.env.AWS_CF_CERT, function (err, pem) {
      var sign = crypto.createSign('RSA-SHA1');
      sign.update(JSON.stringify(policy));
      var signature = sign.sign(pem.toString('ascii'), 'base64');
      var params = [
          'Key-Pair-Id=' + process.env.AWS_CF_KEY,
          'Expires=' + expires,
          'Signature=' + signature
      ];
      callback(err, resource + "?" + params.join('&'));
    });
  } else {
    // S3リダイレクト
    var s3 = this.open(key);
