...
Base.prototype.validator = function (fields, params) {
  ...
  /*
   ...
   var name = field.name;
   var type = field.type;
   var value = util.getValue(params, name);
   var z = typeof value;
   if (value) {
   var new_value = null;
   switch (type) {
   ...
   break;
   case 'Date':
   new_value = new Date(Date.parse(value));
   break;
   case 'ObjectID':
   if (value && value.toString().match(/[a-f0-9]{24}/)) {
   new_value = ObjectID(value.toString());
   }
   break;
   }
   if (new_value)
   util.setValue(params, name, new_value);
   }
   });
   */
};
