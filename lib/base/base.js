...
Base.prototype.do_find = function (context, params, callback) {
  ...
  cursor.count(function (err, total) {
    if (err) {
      ...
      context.total = total; // todo: contextに入れて返すことは、よくないので見直す。しかしコールバックのインタフェースを変更したくない。
...
function _addDefaultSort(params) {
  var sort = params.sort;
  var defaultSort = [SORT_KEY, SORT_ORDER];
  if (Base.prototype.isArray(sort)) {
    var existDefaultSortKey = false;
    // validate
    async.each(sort, function (sortParam, next) {
      var err;
      if (Base.prototype.isArray(sortParam) && sortParam.length == 2) {
...
Base.prototype.isArray = function (o) {
