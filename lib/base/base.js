...
function _addDefaultSort(params) {
  ...
  var defaultSortKey = '_id';
  var defaultSort = [defaultSortKey, -1];
  ...
  if (sort instanceof Array) {
    ...
    // validate
    async.each(sort, function (i, next) {

      var err;
      if (sort[i] instanceof Array && sort[i].length == 2) {

        if (sort[i][0] == defaultSortKey)
          existDefaultSortKey = true;

      } else {
        err = "invalid sort parameter";
      }
      next(err);
